...
export default function LoginPage() {
  ...
  const redirectAfterLogin = (role?: string | null) => {
    window.setTimeout(() => {
      window.location.href = role?.toLowerCase() === "admin" ? "/adminInventory" : "/catalog";
    }, 600);
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (accessToken) {
        ...
        showToast('Login successful', 'Redirecting to your library workspace.', 'success');
      ...
      console.error(err);
      const message = err.data?.message || err.message || "Connection error";
      setErrorMsg(message);
      showToast('Login failed', message, 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] p-4 md:p-8 font-sans relative">
      ...
      <div
        className="fixed bottom-8 right-8 bg-white rounded-full shadow-lg border border-gray-100 p-2 pr-6 flex items-center gap-4 z-50"
        data-aos="fade-up"
        data-aos-delay="150"
      >
        {" "}
      ...
      <div className="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-sm flex flex-col md:flex-row overflow-hidden p-4 md:p-6 gap-8">
        ...
        <div
          className="w-full md:w-1/2 bg-[#FFF5F5] rounded-[2rem] p-8 md:p-12 flex flex-col justify-between relative overflow-hidden"
          data-aos="fade-right"
        >
          {" "}
